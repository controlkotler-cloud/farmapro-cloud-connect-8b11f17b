...
-- 2. Create public pharmacy listings table (no sensitive data) if not exists
...
-- 3. Enable RLS on public pharmacy listings if not already enabled
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables WHERE tablename = 'pharmacy_listings_public' 
...
-- 4. Create policy for public pharmacy listings if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'pharmacy_listings_public' 
    AND policyname = 'Public can view active pharmacy listings'
  ) THEN
    EXECUTE 'CREATE POLICY "Public can view active pharmacy listings" 
...
-- 7. Remove problematic public policies from pharmacy_listings
DROP POLICY IF EXISTS "Public can view active pharmacy listings" ON public.pharmacy_listings;

-- 8. Update pharmacy listings policy to require authentication
...
-- 12. Populate existing pharmacy listings into public table
INSERT INTO public.pharmacy_listings_public (
  id, title, location, description, price, surface_area,
  annual_revenue, is_active, created_at, updated_at, images_urls
)
