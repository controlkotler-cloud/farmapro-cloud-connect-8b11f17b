...
-- 4. Tighten security_audit_log RLS policies
DROP POLICY IF EXISTS "System can insert security audit logs" ON public.security_audit_log;

-- Only allow inserts via the secure function (implicitly through SECURITY DEFINER)
-- or by service role for system operations
CREATE POLICY "Secure audit log inserts only"
...
-- 5. Optional: Create function to get job contact email for authenticated users
