...
-- 1. Fix Security Definer View by removing public views and using proper RLS
DROP VIEW IF EXISTS job_listings_public;
DROP VIEW IF EXISTS pharmacy_listings_public;
...
-- 6. Restrict access to admin_users table (CRITICAL FIX)
...
-- 7. Restrict profiles access (CRITICAL FIX)
...
-- 8. Secure team_members table (CRITICAL FIX)
...
-- 9. Secure premium content access
...
-- 11. Secure forum content - require authentication
...
-- 12. Harden team invitation system
...
-- 13. Populate safe tables with existing data
INSERT INTO public.job_listings_safe (
  id, title, company_name, location, description, requirements,
  salary_range, is_active, expires_at, created_at, updated_at
)
