...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-TEAM-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/dashboard?team_setup=success`,
    ...
    logStep("Team checkout session created", { sessionId: session.id, url: session.url });
    ...
    logStep("ERROR in create-team-checkout", { message: errorMessage });
