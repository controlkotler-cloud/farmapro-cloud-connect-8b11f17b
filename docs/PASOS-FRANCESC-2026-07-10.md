...
- **Prompts Lovable v2** (`prompts-lovable-2026-07-09.md`): reescritos con Holded del portal, email por Mailrelay, metadata origen=portal, fixes manage-team y types.
...
Finder → `~/farmapro/herramientas/skills-rebotica/` → instala los 6 `.skill` (los 4 rebotica-* + `mailrelay-envio` + `impulso-montaje`). La copia que usa Cowork ahora mismo instruye enviar por Clientify con un merge tag roto.
...
Lovable del PORTAL → Settings → Secrets: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `HOLDED_API_KEY` (la de direct), `MAILRELAY_API_KEY` y `MAILRELAY_API_BASE` (están en `herramientas/.mailrelay.env`), `APP_URL` = `https://portal.farmapro.es`.
Luego en Cowork:
